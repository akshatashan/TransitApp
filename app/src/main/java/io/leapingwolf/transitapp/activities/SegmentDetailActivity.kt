...
import io.leapingwolf.transitapp.fragments.MapSegmentInterface
import io.leapingwolf.transitapp.fragments.SegmentDetailListFragment
 ...
 * inflates activity_route_detail which includes content_route_detail */
...
class SegmentDetailActivity : AppCompatActivity(), MapSegmentInterface {
    ...
    private fun  updateFragments(currentPosition: Int) {
        ...
        async() {
            //come back to ui thread for updating the carousel view pager
            uiThread{
                segmentCarouselFragment.updateSegmentDetailList(RouteViewModel(lstRoutes[currentPosition], baseContext,null,hashMapAttributes),currentPosition,lstRoutes[currentPosition])
